...
func LoadImages(containerID, path string) error {
	log.Debug("loading images")
	// list image tars
	findCmd := "find "+path+" -name *.tar"
	loadCmd := "xargs -n1 ctr --namespace kube-system -a "+DefaultContainerdSock+" images import"
	Cmd := findCmd+" | "+loadCmd
